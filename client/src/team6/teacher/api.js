...
export const addCourseStudent = async (courseId, userId) => {
  await apiRequest(`/courses/${courseId}/students`, {
    method: "POST",
    auth: true,
    data: { student_id: String(userId) },
  });
};

export const removeCourseStudent = async (courseId, userId) => {
  await apiRequest(`/courses/${courseId}/students/${userId}`, {
    method: "DELETE",
    auth: true,
    data: {},
  });
...
export const fetchAllUsers = async (limit = 200) => {
  const payload = await apiRequest(`/users?limit=${limit}`, { auth: true });
  return toItems(payload).map(normalizeStudent);
};
